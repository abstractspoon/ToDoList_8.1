									...
									const SEARCHPARAMS& query, BOOL bCheckDueToday, SEARCHRESULT& result) const
{
	...
	for (int nRule = 0; nRule < nNumRules && bMatches; nRule++)
	{
		...
		case TDCA_POSITION:
			...
			bMatch = ValueMatches((pTDS->GetPosition() + 1), rule, sWhatMatched);
		...
		default:
			...
			if (rule.IsCustomAttribute())
			{
				...
				if (query.aAttribDefs.GetAttributeDef(sUniqueID, attribDef))
				{
					...
					bMatch = ValueMatches(data, attribDef.GetAttributeType(), rule, bCaseSensitive, sWhatMatched);
				}
				else
				{
					ASSERT (0);
	...
	HTASKITEM hParentTask, BOOL bIncDuplicateCompletedRecurringSubtasks) const
{
	...
	for (int nSubTask = 0; nSubTask < pTDSParent->GetSubTaskCount(); nSubTask++)
	{
		...
		const TODOITEM* pTDI = m_data.GetTask(pTDS->GetTaskID()); // Not true task
...
BOOL CTDCTaskExporter::ExportTask(DWORD dwTaskID, CTaskFile& tasks, HTASKITEM hParentTask, BOOL bIncDuplicateCompletedRecurringSubtasks) const
{
	...
	if (!m_data.GetTask(dwTaskID, pTDI, pTDS)) // Not true task
	{
		return FALSE;
	}
